...
function board() {
  ...
  const stones = ROUTE_NODES.map((node) => {
    const pos = node.id,
      ...
      occupants = [0, 1].filter((i) => state.players[i].pos === pos),
      wall = state.walls[pos],
      selectedTile = active === pos;
    const shared = occupants.length === 2;
    const occupiedClass = shared
      ? "occupied shared"
      : occupants.length
        ? "occupied " + (occupants[0] === 0 ? "gold" : "blue")
        : "";
    const name = `Stone ${number}${occupants.length ? `, ${occupants.map((i) => state.players[i].name).join(" and ")}${shared ? ", sharing this stone" : ""}` : ""}${wall !== undefined ? `, ${COLORS[wall]} wall` : ""}${legal.has(pos) ? `, available to ${label[state.draft.action].toLowerCase()}` : ", unavailable"}`;
    const pieces = occupants
      .map(
        (i) =>
...
function animateMove(from, entry) {
  const piece = document.querySelector(
    `[data-pos="${entry.target}"] .piece[data-player="${entry.player}"]`,
  );
...
function perform(pass = false) {
  ...
  const previousElement = previousPosition
    ? document.querySelector(
        `[data-pos="${previousPosition}"] .piece[data-player="${state.current}"]`,
      )
